...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    anchor_mask = [[6,7,8], [3,4,5], [0,1,2]] if num_layers==3 else [[3,4,5], [1,2,3]]
    input_shape = tf.cast(K.shape(yolo_outputs[0])[1:3] * 32, K.dtype(y_true[0]))
    grid_shapes = [tf.cast(K.shape(yolo_outputs[l])[1:3], K.dtype(y_true[0])) for l in range(num_layers)]
    loss = 0
    m = K.shape(yolo_outputs[0])[0] # batch size, tensor
    mf = tf.cast(m, K.dtype(yolo_outputs[0]))
    ...
    for l in range(num_layers):
        ...
        def loop_body(b, ignore_mask):
            ...
            ignore_mask = ignore_mask.write(b, tf.cast(best_iou<ignore_thresh, K.dtype(true_box)))
